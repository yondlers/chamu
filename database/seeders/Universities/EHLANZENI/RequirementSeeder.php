...
namespace Database\Seeders\Universities\EHLANZENI;
...
{
    private const PROGRAMMES_PATH = __DIR__.'/ehlanzeni_programmes.json';

    private const PROGRAMMES_SOURCE_URL = 'https://www.ehlanzenicollege.co.za/courses';

    private const WEBSITE = 'https://www.ehlanzenicollege.co.za/';

    private const LOGO = 'https://decisive-serenity-7fc9a48de1.media.strapiapp.com/ehlanzeni_college_logo_208e052816.png';
    ...
    {
        $abbreviation = (string) ($institution['abbreviation'] ?? 'EHLANZENI');
        $name = (string) ($institution['name'] ?? 'Ehlanzeni TVET College');
        ...
        $values = [
            ...
            'name' => $name,
            'slug' => $existing?->slug ?: $this->uniqueUniversitySlug(Str::slug($name) ?: 'ehlanzeni-tvet-college'),
        ...
        if (Schema::hasColumn('universities', 'contact_email')) {
            $values['contact_email'] = $institution['contact_email'] ?? 'info@ehlanzenicollege.co.za';
            $values['contact_phone'] = $institution['contact_phone'] ?? '013 752 7105';
            $values['physical_address'] = $institution['physical_address'] ?? '29 Bell Street, Nelspruit / Mbombela';
    ...
    private function assignSubjectLevelsRule(int $universityId, int $qualificationId, ?int $requiredGradeId): void
    {
        ...
        DB::table('university_admission_rules')->updateOrInsert(
            ...
            [
                ...
                'is_default' => false,
                'overrides' => null,
                'notes' => 'Ehlanzeni matching is based on published school/NQF entry level, subjects, placement testing and college selection criteria rather than APS.',
                'created_at' => now(),
                'updated_at' => now(),
            ],
        );
    ...
    private function notes(array $programme, array $sharedNotes): string
    {
        ...
        $notes[] = 'Minimum requirements do not guarantee admission; Ehlanzeni placement testing, selection criteria and campus capacity may apply.';
    ...
    {
        $base = $base ?: 'ehlanzeni-tvet-college';
        $slug = $base;
        $suffix = 2;
