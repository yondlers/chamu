            ...
            <div class="no-print mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-col gap-2 sm:flex-row">
                    @if ($application->bursary?->source_url)
                        <a href="{{ $application->bursary->source_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 rounded-xl border border-neutral-300 bg-white px-4 py-2.5 text-sm font-black text-neutral-800 hover:bg-neutral-50">
            ...
            <article class="print-sheet rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
                <div class="flex flex-col gap-4 border-b border-neutral-200 pb-6 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-2 text-3xl font-black leading-tight">{{ $application->bursary?->title ?? 'Bursary application' }}</h1>
                        <p class="mt-1 text-base font-bold text-neutral-600">{{ $application->bursary?->company?->name ?? 'Bursary provider' }}</p>
                    </div>
                ...
                <section class="mt-6 grid gap-5 md:grid-cols-2">
                    <div class="rounded-xl border border-neutral-200 p-4">
                        ...
                        @if ($application->bursary?->source_url)
                            <p class="mt-3 text-xs font-semibold text-neutral-500">
                                Source:
                                <a href="{{ $application->bursary->source_url }}" target="_blank" rel="noopener noreferrer" class="break-all text-[#01225E] underline">
                                    {{ $application->bursary->source_url }}
                                </a>
                            </p>
                        @endif
                    </div>
                    ...
                    <ul class="mt-4 space-y-3 text-sm font-semibold text-neutral-800">
                        @forelse ($documents as $document)
                            <li class="flex items-start gap-3 rounded-xl border border-neutral-200 px-3 py-2">
                                ...
                                <span>
                                    <span class="font-black">{{ $document->requirement?->label ?? \Illuminate\Support\Str::of($document->document_key)->replace('_', ' ')->title() }}</span>
