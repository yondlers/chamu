...
@section('title', 'Content · Matric Hub')
...
@section('content')
    <main class="max-w-6xl mx-auto px-5 lg:px-8 py-10">
        <section class="mb-8">
            <p class="text-sm font-semibold text-[#E8425B]">Available content</p>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <span class="rounded-full bg-rose-50 px-3 py-1 text-sm font-semibold text-[#E8425B]">{{ $questions->count() }} grouped questions</span>
            </div>
        </section>
        ...
        @if ($subject === null)
            <section class="rounded-2xl border border-neutral-200 bg-white p-8 soft-card">
                ...
                <a href="{{ url('/') }}" class="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#E8425B] px-5 py-3 font-semibold text-white">
        ...
        @else
            <div class="grid lg:grid-cols-[1fr_320px] gap-6 items-start">
                    ...
                    @foreach ($questionsBySource as $source => $sourceQuestions)
                        <article class="rounded-2xl border border-neutral-200 bg-white p-6 soft-card">
                            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                <div>
                                    <p class="text-sm font-semibold text-[#E8425B]">Source</p>
                                ...
                                @auth
                                    <form method="GET" action="{{ route('practice.setup') }}">
                                        ...
                                        <button class="inline-flex items-center gap-2 rounded-xl bg-[#E8425B] px-4 py-2.5 text-sm font-semibold text-white">
                                            Do this source <i data-lucide="arrow-right" style="width:16px;height:16px;"></i>
                                        </button>
                                    </form>
                                @else
                                    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 rounded-xl bg-[#E8425B] px-4 py-2.5 text-sm font-semibold text-white">Log in to practise</a>
                                @endauth
                            </div>
                ...
                <aside class="lg:sticky lg:top-24 rounded-2xl border border-neutral-200 bg-white p-5 soft-card">
                    <span class="inline-flex w-10 h-10 items-center justify-center rounded-xl bg-rose-50 text-[#E8425B] mb-4">
                    ...
                    @auth
                        <form method="GET" action="{{ route('practice.setup') }}" class="mt-5">
                            ...
                            <button class="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#E8425B] px-4 py-3 text-sm font-semibold text-white">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="mt-5 w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#E8425B] px-4 py-3 text-sm font-semibold text-white">Log in to start</a>
                    @endauth
                </aside>
            </div>
        @endif
