...
@section('title', 'Profile · Matric Hub')

@section('content')
    <main class="max-w-5xl mx-auto px-5 lg:px-8 py-10">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
            <div>
                <p class="text-sm font-semibold text-[#E8425B]">Account</p>
            ...
            <section class="rounded-2xl border border-neutral-200 bg-white p-6 soft-card">
                ...
                <div class="grid md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input id="first_name" name="first_name" value="{{ old('first_name', $user->first_name) }}" required class="w-full rounded-xl border border-neutral-300 px-4 py-3 outline-none focus:border-[#E8425B]">
                    ...
                    <div>
                        ...
                        <input id="last_name" name="last_name" value="{{ old('last_name', $user->last_name) }}" class="w-full rounded-xl border border-neutral-300 px-4 py-3 outline-none focus:border-[#E8425B]">
                    ...
                    <div>
                        ...
                        <input id="username" name="username" value="{{ old('username', $user->username) }}" required class="w-full rounded-xl border border-neutral-300 px-4 py-3 outline-none focus:border-[#E8425B]">
                        @error('username') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-semibold mb-2">Email</label>
                        <input id="email" name="email" type="email" value="{{ old('email', $user->email) }}" required class="w-full rounded-xl border border-neutral-300 px-4 py-3 outline-none focus:border-[#E8425B]">
        ...
        <section class="mt-8 grid md:grid-cols-2 gap-5">
            <a href="{{ route('subjects.index') }}" class="rounded-2xl border border-neutral-200 bg-white p-6 soft-card hover:border-[#E8425B]">
                <span class="inline-flex w-11 h-11 items-center justify-center rounded-xl bg-rose-50 text-[#E8425B] mb-4">
            ...
            <a href="{{ route('marks.index') }}" class="rounded-2xl border border-neutral-200 bg-white p-6 soft-card hover:border-[#E8425B]">
                <span class="inline-flex w-11 h-11 items-center justify-center rounded-xl bg-rose-50 text-[#E8425B] mb-4">
